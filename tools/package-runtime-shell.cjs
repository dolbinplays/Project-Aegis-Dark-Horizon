...
const PWA_UPDATE_RELOAD_GUARD_KEY="project-aegis-pwa-update-reload-v1";
  const PWA_UPDATE_CHECK_TIMEOUT_MS=2600;
  const PWA_UPDATE_ACTIVATION_TIMEOUT_MS=12000;
  const pwaState={deferredPrompt:null,installed:false,serviceWorker:"unsupported",lastOutcome:null,updatePhase:"idle",startupUpdateDetected:false,startupReloaded:false};
  ...
  function pwaSecureEligible(){const protocol=window.location?.protocol||"";return window.isSecureContext===true||protocol==="http:"&&/^(localhost|127\.0\.0\.1)$/i.test(location.hostname||"");}
  function getPwaState(){const protocol=window.location?.protocol||"";return{installed:pwaInstalled()||pwaState.installed,canPrompt:Boolean(pwaState.deferredPrompt&&!pwaInstalled()),displayMode:pwaDisplayMode(),secure:pwaSecureEligible(),protocol,ios:/iphone|ipad|ipod/i.test(navigator.userAgent||""),serviceWorker:pwaState.serviceWorker,lastOutcome:pwaState.lastOutcome,updatePhase:pwaState.updatePhase,startupUpdateDetected:pwaState.startupUpdateDetected,startupReloaded:pwaState.startupReloaded};}
  ...
  function registerAegisServiceWorker(){if(pwaRegistrationPromise)return pwaRegistrationPromise;if(!("serviceWorker" in navigator)){pwaState.serviceWorker="unsupported";notifyRuntimePwaState();return Promise.resolve(null);}if(!pwaSecureEligible()){pwaState.serviceWorker="insecure-context";notifyRuntimePwaState();return Promise.resolve(null);}pwaState.serviceWorker="registering";notifyRuntimePwaState();pwaRegistrationPromise=navigator.serviceWorker.register("./service-worker.js",{scope:"./",updateViaCache:"none"}).then(registration=>{pwaState.serviceWorker="registered";notifyRuntimePwaState();return registration;}).catch(error=>{pwaState.serviceWorker="failed";console.warn("AEGIS PWA service worker registration failed:",error);notifyRuntimePwaState();return null;});return pwaRegistrationPromise;}
  ...
  window.__AEGIS_PWA_UPDATE_REPORT=()=>({build:BUILD,...getPwaState(),hasController:Boolean(navigator.serviceWorker?.controller),reloadGuard:pwaReloadGuardMatches?.()===true,checkTimeoutMs:PWA_UPDATE_CHECK_TIMEOUT_MS,activationTimeoutMs:PWA_UPDATE_ACTIVATION_TIMEOUT_MS});
  function pwaDelay(ms){return new Promise(resolve=>setTimeout(resolve,Math.max(0,Number(ms)||0)));}
  function pwaReloadGuardMatches(){try{const value=JSON.parse(sessionStorage.getItem(PWA_UPDATE_RELOAD_GUARD_KEY)||'null');return Boolean(value&&value.build===BUILD&&Date.now()-Number(value.at||0)<5*60*1000);}catch{return false;}}
  function markPwaReloadGuard(){try{sessionStorage.setItem(PWA_UPDATE_RELOAD_GUARD_KEY,JSON.stringify({build:BUILD,at:Date.now()}));}catch{}}
  async function prepareInstalledPwaStartup(){const installedAtLaunch=pwaInstalled();const controllerAtLaunch=navigator.serviceWorker?.controller||null;const hadController=Boolean(controllerAtLaunch);const registration=await Promise.race([registerAegisServiceWorker(),pwaDelay(PWA_UPDATE_CHECK_TIMEOUT_MS).then(()=>null)]);if(!registration){pwaState.updatePhase="fallback";notifyRuntimePwaState();return{reloading:false};}if(!installedAtLaunch||!hadController){pwaState.updatePhase="background-check";registration.update?.().catch(()=>{});notifyRuntimePwaState();return{reloading:false};}
    pwaState.updatePhase="checking";notifyRuntimePwaState();let updateDetected=Boolean(registration.installing||registration.waiting);let controllerChanged=false;let resolveControllerChange=null;const controllerChangePromise=new Promise(resolve=>{resolveControllerChange=resolve;});const onControllerChange=()=>{controllerChanged=true;resolveControllerChange?.(true);};const onUpdateFound=()=>{updateDetected=true;pwaState.startupUpdateDetected=true;pwaState.updatePhase="installing";showTransition('UPDATING AEGIS COMMAND NETWORK','A new command build was detected. Installing it before game startup...','One-tap update handoff \u2022 current campaign data remains untouched');notifyRuntimePwaState();};navigator.serviceWorker.addEventListener("controllerchange",onControllerChange);registration.addEventListener("updatefound",onUpdateFound);if(updateDetected)onUpdateFound();try{await Promise.race([Promise.resolve(registration.update?.()),pwaDelay(PWA_UPDATE_CHECK_TIMEOUT_MS)]);}catch(error){console.warn("AEGIS PWA startup update check failed:",error);}if(registration.installing||registration.waiting){if(!updateDetected)onUpdateFound();try{registration.waiting?.postMessage?.({type:"AEGIS_SKIP_WAITING"});}catch{}}if(navigator.serviceWorker.controller&&navigator.serviceWorker.controller!==controllerAtLaunch){controllerChanged=true;if(!updateDetected)onUpdateFound();}
    if(updateDetected){const changed=controllerChanged||await Promise.race([controllerChangePromise,pwaDelay(PWA_UPDATE_ACTIVATION_TIMEOUT_MS).then(()=>false)]);registration.removeEventListener("updatefound",onUpdateFound);navigator.serviceWorker.removeEventListener("controllerchange",onControllerChange);if(changed&&!pwaReloadGuardMatches()){pwaState.updatePhase="reloading";pwaState.startupReloaded=true;markPwaReloadGuard();transitionStatus.textContent='Update installed. Reopening AEGIS on the new command build...';transitionDetail.textContent='Automatic handoff \u2022 no second app launch required';notifyRuntimePwaState();setTimeout(()=>window.location.reload(),120);return{reloading:true};}pwaState.updatePhase=changed?"ready":"timeout";notifyRuntimePwaState();if(rebooting)showTransition('RESTORING MISSION DEBRIEF','Loading the verified post-mission autosave...','After Action Report \\u2022 command continuity');else hideTransition(140);return{reloading:false};}
    registration.removeEventListener("updatefound",onUpdateFound);navigator.serviceWorker.removeEventListener("controllerchange",onControllerChange);pwaState.updatePhase="current";notifyRuntimePwaState();return{reloading:false};}
  ...
  (async()=>{const update=await prepareInstalledPwaStartup();if(update?.reloading)return;bootRuntime();})();
})();
</script>
</body>
</html>
