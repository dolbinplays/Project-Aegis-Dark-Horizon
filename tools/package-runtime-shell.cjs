...
(function(){
  ...
  function validResumeToken(token){const age=Date.now()-Number(token?.createdAt||0);return Boolean(token&&token.kind==='project-aegis-post-mission-runtime-resume'&&token.build===BUILD&&token.autosaveSlot&&token.reportId&&token.checkpointId&&age>=0&&age<=RESUME_TOKEN_TTL_MS);}
  ...
  function runtimeResumeComplete(info={}){if(lastVerifiedToken&&(info.reportId!==lastVerifiedToken.reportId||info.checkpointId!==lastVerifiedToken.checkpointId))return runtimeResumeFailed({message:'The replacement runtime restored a different mission checkpoint.'});clearResumeWatchdog();metrics.resumes+=1;metrics.lastResume={...info,at:Date.now()};lastVerifiedToken=null;transitionTitle.textContent='AFTER ACTION REPORT READY';transitionStatus.textContent='Campaign restored. Opening the completed mission report...';transitionDetail.textContent='Tactical runtime reset complete \u2022 campaign continuity verified';setTimeout(()=>{hideTransition(0);rebooting=false;},520);}
  function runtimeResumeFailed(info={}){clearResumeWatchdog();metrics.resumeFailures+=1;rebooting=false;transitionTitle.textContent='RECOVERY CHECK REQUIRED';transitionStatus.textContent=String(info.message||'The post-mission autosave could not be restored.');transitionDetail.textContent='The verified autosave remains available. Retry it or continue to the start screen.';recoveryActions.classList.add('active');transition.setAttribute('aria-busy','false');transition.classList.add('active');if(lastAudioState.musicPlaying)fadeBridge(targetBridgeVolume(lastAudioState),220);}
