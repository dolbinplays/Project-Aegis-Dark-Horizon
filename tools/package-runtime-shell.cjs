...
(function(){
  ...
  const PWA_UPDATE_STRATEGY="release-beacon-navigation-v2";
  ...
  window.__AEGIS_PWA_UPDATE_REPORT=()=>({build:BUILD,...getPwaState(),hasController:Boolean(navigator.serviceWorker?.controller),strategy:PWA_UPDATE_STRATEGY,blockingStartupGate:false,automaticControllerReload:false});
  function refreshAegisServiceWorkerInBackground(){pwaState.updatePhase="background-check";notifyRuntimePwaState();registerAegisServiceWorker().then(registration=>{if(!registration){pwaState.updatePhase="fallback";notifyRuntimePwaState();return;}registration.update?.().catch(()=>{});pwaState.updatePhase="ready";notifyRuntimePwaState();}).catch(()=>{pwaState.updatePhase="fallback";notifyRuntimePwaState();});}
  ...
  // Boot the embedded game immediately. Shell freshness is decided by the service worker's
  // tiny release-beacon navigation path before this host is delivered; service-worker upkeep
  // must never block an ordinary Android cold start or reload an active mission.
  bootRuntime();
  setTimeout(refreshAegisServiceWorkerInBackground,0);
})();
</script>
</body>
</html>
