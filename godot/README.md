...
Native build: `v0.26.08.03.GODOT.0026_CROSS_SQUAD_DIRECT_CONTACT_RESPONSE_VERTICAL_SLICE`

Paired browser build: `v0.26.08.03.0156_CROSS_SQUAD_DIRECT_CONTACT_RESPONSE_PARITY_PATCH`
...
- Building-aware escort routing selects a real door or destroyed-wall breach before heading to extraction, then continues through the ramp until the full trailing civilian column is aboard.
- Mandatory-rescue civilians carry periodic VIP tracker pulses. Before alien contact, every free AI soldier independently follows that soldier's closest unescorted tracker target; after all tracked civilians are located, searchers resume distinct building-interior and map-sector sweeps.
- Any squad-known alien contact pauses new civilian claims and redirects every non-escort to combat. Established escorts continue evacuating, while a free soldier that reaches an adjacent civilian immediately spends the contact TU and locks the escort instead of orbiting the target.
...
- A living original alien commander can make one low, escalating reinforcement call after observing soldiers. Arrival is delayed two rounds and deploys two to four aliens from a small purple dropship.
- If the commander dies without calling, a deterministic 5-to-15-round missed-check-in window begins. Wiping the original force while an objective remains unresolved draws one investigation force when that deadline expires.
- Commander calls and missed-check-in investigations share one reinforcement allowance. The investigation force arrives immediately when due, subject to the same bounded clear-landing retry.
- Alien dropship placement checks at most 32 deterministic perimeter candidates. Hull and ramp cells remain inside the map, separated from buildings and every Skyranger hull/ramp, and cannot overlap a living unit.
- Alien landing craft render as layered purple flying saucers with a rear deployment opening, illuminated rim, and usable ramp.
...
- Cross-squad response distinguishes a mission-wide report from personal sight. Non-escorts advance by the shortest practical bounded route to the nearest reported or last-known contact, then switch to cover, formation, range, and firing decisions as soon as they personally see an alien.
...
- Functional field inventory transfers hand and belt items between adjacent same-elevation soldiers for 4 TU, preserves exact floor cell/elevation on drop and pickup, and blocks primed-grenade or cross-elevation transfers.
- Fit Map reframes the complete Small, Medium, or Large battlefield inside the tactical board.
...
- Prepaid concurrent construction for Living Quarters, Laboratories, and Workshops using one compact facility dropdown and the browser campaign's established $300k, $450k, and $400k costs.
...
- In-game Build Health with 104 checks, including direct cross-squad contact response, commander-call and missed-check-in reinforcement ownership, purple saucer rendering, clear dropship placement, nearest-VIP routing, doorway ingress without circling, building-aware escort egress, full-column ramp extraction, squad-wide rescue/combat handoff, all-soldier tracker guidance, Fit Map framing, adjacent inventory transfer/floor elevation, compact facility construction, escorted-VIP visibility, rescue perimeter guards, biome/tier building density, map tiers, exact multi-Skyranger squad deployment, playable-perimeter guards, air operations, browser-import isolation, personnel, research/manufacturing/construction, local-stock conservation, Medkits, wounds, commander doctrine, reaction fire, fog, AI reclaim, sequential visible actors, queued voices, classic command controls, and mission recovery/loss.
...
- Use **Prev/Next**, **Map**, **Fit Map**, and **Inventory** to inspect the squad and observation-safe battlefield contacts. Inventory can give, drop, and pick up eligible equipment from the selected soldier's exact cell and elevation.
...
The test runner covers campaign creation and travel, exact loadout/store save round-tripping, conservative loadout and wound migration, local-stock conservation and unavailable-item rejection, Medkit issue/return/consumption, mission recovery and loss, one-to-five-day wounds, exact recovery persistence, unequipped recruit arrivals, Workshop and personnel migration, hiring/capacity rules, construction, bounded research and manufacturing, air operations, browser-export normalization and source preservation, strategic markers, bounded hex rules, tactical loadout inheritance, Laser Rifle range/TU, selected-unit feedback, Unarmed fire rejection, movement highlighting, three End Turn cycles, civilian contact, wall destruction and traversal, report-versus-personal-sight separation, direct cross-squad convergence and engagement handoff, full-squad contact response, per-soldier nearest-VIP priority, escorted-VIP visibility, distinct rescue guards, biome/tier building density, remembered alien-contact convergence, building-clear Skyranger placement, tracked VIP pings, commander-call reinforcement, deterministic missed-check-in timing, wiped-force investigation arrival, one-wave ownership, building- and Skyranger-clear alien saucer placement, two-to-four reinforcement deployment, threat-safe civilian paths, sequential visible actions, and all visible Build Health rows.

Latest automated verification passes `147/147` native tests and `104/104` visible Build Health rows. Godot 4.7.1 strict editor parsing passes. Paired browser build 0156 passes six-script static parsing, the build seam checker, and `327/327` localhost Build Health. A six-soldier 64x64 mission completed three confirmed End Turn cycles; tactical-map AI command generated a 33-frame continuation, Take Back Control restored the human phase, and no browser runtime errors were recorded.
...
The paired 0023/0153 gate requires a rescue battle with at least one VIP inside a building. Before alien contact, confirm free soldiers follow tracker pings, use a real doorway, and contact an adjacent VIP without circling. After alien contact, confirm new claims pause while existing escorts continue. Exercise Fit Map on every map tier and transfer, drop, and recover one item between adjacent soldiers on the same elevation.

The paired 0024/0154 gate requires killing the alien commander before a call and confirming that continued alien observation cannot summon reinforcements. In separate Medium and Large missions, including a two-Skyranger deployment, allow the call and confirm the two-round delay, one-call limit, two-to-four arrivals, and purple dropship separation from every building and both Skyrangers. A separated-VIP rescue should also confirm each free soldier independently contacts the closest unescorted VIP.

The paired 0025/0155 gate requires killing the original commander before a call, wiping the original force while a mandatory rescue remains unresolved, and confirming that one investigation saucer arrives at the deterministic 5-to-15-round deadline. Complete an equivalent mission before the deadline to confirm no arrival, then allow a normal commander call in a separate mission to confirm the shared one-wave allowance. Inspect the saucer, rear opening, and ramp in browser 2D, browser Three.js, and Godot, including Medium and Large maps with two Skyrangers.

The paired 0026/0156 gate requires a widely separated two-squad incident under AI command. Let Squad A report aliens while Squad B has no personal sight. Confirm every non-escort in Squad B takes a direct route toward the reported or last-known position, then switches to cover and engagement behavior immediately after gaining personal sight. Existing civilian and VIP escorts must continue extraction instead of joining the response.

Next paired patch after this gate: `TACTICAL_CLASSIC_INVENTORY_CONTAINER_LOADOUT_AND_ELEVATION_RENDER_FOUNDATION_PARITY_PATCH`, expanding the bounded inventory foundation without beginning a broad multi-level tactical rewrite.
