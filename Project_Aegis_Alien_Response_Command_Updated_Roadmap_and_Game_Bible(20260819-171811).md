...
0b. **Smooth Geoscape time**
   ...
   - Future presentation pass: aircraft and UFOs should move continuously between their authoritative route positions rather than visibly stepping once per strategic tick. See **Future Roadmap Addition — Smooth Geoscape Craft Flight Animation** below.
...
### Future Roadmap Addition — Concurrent Independent Skyranger Incident Responses
Status: **Roadmapped / strategic air-operations architecture expansion**

Design goal:
- AEGIS should be able to respond to more than one ground incident at the same time when it owns enough available transports and squads. **Skyranger A being committed to Incident A must not prevent Skyranger B from launching to Incident B.** Transport availability should be determined per aircraft, not by a campaign-wide "a Skyranger operation already exists" lock.

Core strategic doctrine:
- Every Skyranger is an independently ownable strategic asset with its own readiness, home base/hangar, squad manifest, route, fuel plan, ferry/staging reservations, outbound phase, tactical ownership, return route, repair/refuel state, and operation identity.
- A Skyranger is unavailable only when **that aircraft** is already claimed by an operation, transfer, repair/refuel restriction, or another legitimate per-aircraft blocker.
- The existence of another outbound Skyranger, another active Skyranger travel record, or another committed incident must not by itself block launch of an otherwise Ready transport.
- Mission Control should be able to launch Incident B while Incident A is already outbound, in tactical preparation, under Simulation AI, awaiting player intervention, or returning home, provided Incident B has its own legal squad/transport resources.
- Multiple simultaneous operations may originate from the same base or different bases, subject to real squad location, hangar ownership, fuel, range, ferry-network, and staging-capacity rules.

Relationship to existing multi-Skyranger doctrine:
- Preserve the existing **atomic multi-Skyranger launch** system when two or more transports are intentionally assigned to the **same incident**. Those transports remain one coordinated operation with one transaction and coherent tactical deployment ownership.
- Add a separate **concurrent-operation authority** for transports assigned to **different incidents**. Independent operations must never be merged merely because their aircraft are airborne at the same time.
- Existing permanent `homeBase` / `homeHangarKey` ownership, staged per-leg fuel accounting, ferry/refuel routing, and correct home-return behavior remain authoritative for every aircraft independently.

Data / ownership architecture:
- Replace any remaining singleton assumption such as one global `skyrangerTravel`, one globally blocking committed transport operation, or a blanket `activeMission` launch lock with an operation collection keyed by stable operation/incident identity.
- Each operation should own its exact `aircraftIds`, response squads/soldiers, incident ID, outbound/arrival/tactical/return phase, route/fuel data, timestamps/ETAs, and save/reload state.
- Aircraft lookup should answer "which operation owns this craft?" rather than "is any Skyranger operation active?"
- Incident lookup should answer "is this incident already claimed by an operation?" so two independent launches cannot accidentally duplicate the same incident unless the player is explicitly adding another transport to the same coordinated response before commit.
- Save migration should preserve legacy single-operation saves by wrapping the old authoritative travel/mission record into the new collection rather than discarding or duplicating aircraft ownership.

Ferry / staging behavior:
- Concurrent operations may use the ferry network at the same time.
- Forward-base hangars/staging slots must be reserved per route leg so two unrelated Skyrangers cannot silently occupy the same transient capacity.
- One operation waiting to refuel at a staging base must not freeze another operation whose route is otherwise clear.
- Dateline shortest-route logic and future smooth craft-flight interpolation apply independently to every active Skyranger.

Mission / tactical handoff behavior:
- Each arriving operation retains its own tactical mission state. Completing, withdrawing from, retrying Simulation AI for, or taking manual control of Incident A must not mutate Incident B's aircraft or mission ownership.
- If more than one operation becomes tactically ready, the UI should preserve every ready operation rather than overwriting a single `activeMission` slot. A player can select which ready mission to enter when the campaign flow permits.
- Simulation-AI ownership, streamed continuation/recovery, victory commit, and return-to-base behavior must remain scoped to that operation's mission and transports.
- A failed/aborted operation affects only its assigned aircraft/squads and incident consequences; it must not release, recall, or resolve unrelated transports.

Geoscape / UI presentation:
- Show all active Skyranger operations simultaneously on Globe and Terminator Map, with each craft following its own route and phase.
- The aircraft/route panel should list concurrent responses individually with incident name, assigned craft, phase, ETA, origin/home base, and return status.
- Mission cards should distinguish **Available**, **Response En Route**, **Tactical Ready / In Progress**, and **Resolved/Returning** without treating any one response as a global transport lock.
- Launch blockers should name the actual unavailable aircraft/squad/resource condition. Do not show "another Skyranger operation is active" when a different Ready transport can legally respond.

Regression / Build Health requirements:
1. **Two incidents + two Ready Skyrangers** -> launch Skyranger A to Incident A, then launch Skyranger B to Incident B while A remains outbound.
2. **One committed + one Ready** -> only the committed aircraft is unavailable; the Ready aircraft remains selectable for another incident.
3. **Two transports intentionally assigned to one incident** -> existing atomic same-operation launch remains intact and does not become two independent operations.
4. **Different bases** -> concurrent responses can launch from their real home/boarding bases and retain independent ferry/fuel plans.
5. **Concurrent staged routes** -> staging-hangar reservations cannot collide or steal ownership between operations.
6. **Save/reload with multiple flights** -> every operation, route, aircraft owner, incident owner, fuel state, and ETA restores without collapsing into one record.
7. **One mission resolves first** -> its Skyranger(s) return normally while the other operation continues untouched.
8. **Simulation AI on one operation** -> AI continuation/retry/victory state cannot block launch or corrupt ownership of another Ready Skyranger operation.
9. **Incident de-duplication** -> an already committed incident cannot accidentally receive a second independent response unless explicitly expanded before launch under the coordinated multi-transport path.

Recommended staged implementation:
1. **Operation collection foundation** — introduce per-operation strategic ownership while migrating the current singleton travel/mission record safely.
2. **Independent launch authority** — remove global Skyranger-flight blockers and validate availability per aircraft/incident.
3. **Concurrent route/ferry progression** — tick all active operations independently with staging-slot reservations.
4. **Mission Control / Geoscape UI parity** — display and select multiple simultaneous operations clearly.
5. **Tactical/Simulation handoff parity** — preserve independent mission-ready, AI, victory, withdrawal, and return states.
6. **Save/load + stress coverage** — exercise several overlapping outbound, tactical-ready, returning, and staged operations.

Suggested future patch names:
- `SKYRANGER_MULTI_OPERATION_OWNERSHIP_FOUNDATION`
- `SKYRANGER_CONCURRENT_INCIDENT_RESPONSE_AND_ROUTE_PARITY`
...
Presentation doctrine:
- Keep route progress, ETA, fuel use, ferry stops, interception timing, return-to-base timing, repairs, refueling, incidents, and mission arrival on the existing authoritative Geoscape clock. Smooth flight is a presentation interpolation layer, not a second travel simulation.
- Each visible craft should interpolate from its last authoritative route position toward the next expected authoritative position every animation frame. A normal strategic tick updates the route anchor without snapping the marker.
- Apparent flight speed must scale naturally with the selected Geoscape compression rate. At slow speeds the craft should drift gently; at high speeds it should cross large distances quickly but still move continuously rather than teleporting between tick positions.
- Pause freezes craft exactly where they are visually. Resume continues from that displayed position without a jump. Changing clock speed changes interpolation rate without resetting craft position.
- Globe and Terminator Map must consume the same interpolated longitude/latitude for each craft so switching views never makes an aircraft or UFO jump to a different route position.
- Preserve the Browser 0045 stable-component lifecycle doctrine: smooth craft animation must not reintroduce Geoscape remount/flicker behavior. Heavy WebGL/canvas renderers should remain persistent across strategic ticks.

Route and world-wrap behavior:
- Interpolation must follow the same shortest-route/dateline authority already used by strategic aircraft pathing. A craft traveling from 170°E to 170°W should smoothly cross the nearby map seam rather than sweeping across 340° of longitude.
- On the Terminator Map, seam-crossing craft should leave one edge and enter the opposite edge continuously, using presentation duplicates only when necessary. There must still be only one authoritative aircraft/UFO state.
- On the Globe, interpolation follows the spherical great-circle/route arc and remains visually attached to the Earth surface at the fixed Globe radius.
- Ferry/staging routes should interpolate leg by leg and visibly pause/refuel only when the authoritative route actually calls for a stop.

Craft types covered:
- Skyrangers and future troop transports.
- Interceptors, including outbound attack runs and return legs.
- UFOs in detected flight, evasive movement, landing approaches, departures, and future terror/mission-intent routes.
- Future alien transports, larger invasion craft, and other strategic vehicles should inherit the same interpolation authority rather than implementing bespoke tick-stepped motion.

Readability and interaction:
- Selection rings, labels, range/ferry overlays, intercept vectors, and click/hit targets should track the interpolated craft position rather than lagging one tick behind the visible marker.
- Movement should remain readable at high compression. If necessary, use bounded motion blur/trails or a short vector tail, but do not obscure markers or turn the Geoscape into a particle-heavy display.
- Craft animation should pause or reduce appropriately when blocking campaign modals, tactical missions, council slides, or other states intentionally stop Geoscape time.
...
- `GEOSCAPE_SMOOTH_CRAFT_FLIGHT_AND_WORLD_WRAP_PARITY`
- `GEOSCAPE_CRAFT_ROUTE_TRANSITION_AND_HEADING_POLISH`
