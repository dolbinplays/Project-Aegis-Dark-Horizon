...
(()=>{'use strict';
 ...
 const enabled=!!host,views=new Set();
 ...
 api.frameDue=(state,now)=>{if(!enabled)return true;if(document.hidden)return false;if(state.tvLastFrame!==undefined&&now-state.tvLastFrame<1000/30)return false;state.tvLastFrame=now;return true;};
 api.createRenderer=(THREE,options)=>{
  ...
  const draw=()=>{timer=0;if(disposed||!pending)return;const args=pending;pending=null;lastDraw=performance.now();render(...args);view.frames++;};
  renderer.render=(...args)=>{
   if(disposed)return;
   if(renderer.getRenderTarget?.())return render(...args);
   pending=args;const wait=1000/30-(performance.now()-lastDraw);
   if(wait<=0){clearTimeout(timer);draw();}else if(!timer)timer=setTimeout(draw,wait);
  };
  renderer.dispose=()=>{disposed=true;clearTimeout(timer);pending=null;views.delete(view);dispose();};
  return renderer;
 };
 ...
 window.addEventListener('pagehide',()=>{clearInterval(heartbeat);observer?.disconnect();if(host.AEGIS_TV_ACTIVE_RUNTIME===api)host.AEGIS_TV_ACTIVE_RUNTIME=null;});
})();
