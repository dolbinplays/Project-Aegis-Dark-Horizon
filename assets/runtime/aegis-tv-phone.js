...
let peer=null,connection=null,ready=false,drag=false,lastPong=0,dx=0,dy=0,raf=0,joinTimer=0;
const status=text=>$('status').textContent=text;
function send(action){if(ready&&connection?.open)connection.send({type:'input',action:P.normalize(action)});}
function release(){send({kind:'release'});drag=false;$('drag').setAttribute('aria-pressed','false');$('drag').textContent='Drag: off';dx=dy=0;}
function stop(message){release();ready=false;const old=peer;peer=null;connection=null;old?.destroy();clearTimeout(joinTimer);$('pairForm').hidden=false;$('controls').hidden=true;$('join').disabled=false;status(message);}
...
setInterval(()=>{if(ready&&connection?.open){connection.send({type:'ping'});if(Date.now()-lastPong>6000)stop('TV connection was interrupted. Reconnect to continue.');}},1000);
function flush(){raf=0;if(dx||dy){send({kind:'move',dx,dy});dx=dy=0;}}
const pointers=new Map();let traveled=0,multi=false;
$('pad').onpointerdown=e=>{e.preventDefault();if(!pointers.size){traveled=0;multi=false;}else multi=true;pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});$('pad').setPointerCapture(e.pointerId);$('pad').classList.add('active');};
...
$('drag').onclick=()=>{drag=!drag;send({kind:drag?'down':'up'});$('drag').setAttribute('aria-pressed',String(drag));$('drag').textContent=drag?'Drag: ON':'Drag: off';};
...
document.addEventListener('visibilitychange',()=>{release();pointers.clear();if(!document.hidden&&ready){lastPong=Date.now();status('Controller active.');}});window.addEventListener('pagehide',()=>stop('Disconnected.'));
})();
