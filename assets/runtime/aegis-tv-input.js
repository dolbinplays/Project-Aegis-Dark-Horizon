...
root.AEGIS_CREATE_TV_INPUT=function(frame,cursor,notify=()=>{}){
 ...
 function click(){if(held)release();const t=hit();if(!t)return;const el=t.el.closest('button,input,select,textarea,a')||t.el;if(el.disabled||el.matches?.(':disabled')||el.getAttribute?.('aria-disabled')==='true')return;t.el=el;el.focus?.({preventScroll:true});
 ...
 function input(raw){const action=root.AEGIS_TV_PROTOCOL.normalize(raw);if(!action)return;try{
  ...
  if(action.kind==='key'){
   ...
   const target=t.doc.activeElement||t.el;if(target.disabled||target.matches?.(':disabled')||target.getAttribute?.('aria-disabled')==='true')return;const down=new w.KeyboardEvent('keydown',{key:action.key,bubbles:true,cancelable:true});target.dispatchEvent(down);const up=new w.KeyboardEvent('keyup',{key:action.key,bubbles:true,cancelable:true});target.dispatchEvent(up);if(action.key==='Enter'&&!down.defaultPrevented&&!up.defaultPrevented&&target.isConnected&&target.matches('button,a[href]'))target.click();
  }
