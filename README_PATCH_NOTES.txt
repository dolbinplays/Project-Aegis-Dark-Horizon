...
BUILD: v0.26.09.14.1112_BATTLE_VISIBILITY_REUSE_PERFORMANCE_PATCH
...
BUILD: v0.26.09.14.0955_SOLID_BUILDING_WALL_VISIBILITY_PATCH
...
BUILD: v0.26.09.13.1807_VEHICLE_BODY_FOOTPRINT_AND_PLAYBACK_PATH_INTEGRITY_PATCH
...
BUILD: v0.26.09.13.1726_VITALS_SELECTION_AND_ITEMIZED_MISSION_REPORTS_PATCH
...
Summary
- Adds a Mobile · Landscape option on the start screen and in Menu / Save. Standard remains the default and preserves the classic PC interface. The choice is stored on this device, outside campaign saves.

Key changes
- Tactical navigation, soldier selection, and camera controls use the left thumb rail; combat, equipment, and playback controls use the right. Both rails scroll on shorter phones.
- The center is reserved for the battlefield. Equipment, extra controls, and detailed status open in scrollable drawers with close buttons.
- Dialogs and tutorials stay inside the available viewport. The fire-team command map scrolls without collapsing its map below a tall header.
- Mobile renderer sizing follows the actual canvas container and reacts to rotation/resizing. Auto quality chooses Performance; explicit Quality remains available.
- Host safe areas and dynamic viewport sizing accommodate phone cutouts and browser controls. Mobile tactical touch handling guards pointer-capture races and suppresses drag/multiple-touch clicks.
- Incorporates the September 5 Geoscape pointer-capture hotfix into canonical source so packaging retains it.
- Save format remains 4. Native Godot mobile interface parity is deferred explicitly in the manifest.

Validation
- 9 mobile behavioral tests and 16 playback sequencer tests pass, including interrupted and prolonged two-finger tactical gestures.
- Phone-sized browser checks cover layout persistence, soldier selection, equipment and orders windows, viewport resizing, and switching back to Standard.
- Seeded browser Build Health matches the previous packaged release: 783/845 pass, with the same 62 existing failures and no added failures.
- Embedded JavaScript, source/package byte identity, build lineage, and synchronized metadata checks pass.
- Physical iOS/Android gesture, cutout, and performance acceptance remains a device playtest; no measured phone FPS gain is claimed.
